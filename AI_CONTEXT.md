...
### 8.1.6 後端商品快取（CacheService）反而讓「資料 API」更不穩定，已移除；且「測一次就判斷排除」是錯的

8.1.5 修好逾時／重試之後，Apps Script 專案（`Line_bot_V1_FAYE`）那邊
另外加了 `ProductRepository.getAll()` 的跨執行快取（`CacheService`，
2 分鐘 TTL），用意是減少重複讀 514 列 PRODUCT_MASTER 的次數。但
8.1.5 已經用 Apps Script 執行紀錄證明 `doGet` 本來就只要 2.18 秒，
資料量根本不是問題——這個快取其實是在「本來就夠快」的路徑上，
多加了一個同樣會打 Google 服務、同樣可能延遲失敗的環節。

**用「連續開好幾次、跨過 2 分鐘快取 TTL」實測後才發現**：有快取時
會出現「前 3～4 次正常，之後開始失敗」的模式；把快取整段拿掉、
只留其他 5 個改動後，連續測 10 次只失敗 1 次（跟 8.1.5 講的殘留網路
路由延遲相符）。**結論：這個快取機制弊大於利，已經永久移除，不要
再加回來。**

這裡我（AI）犯了一次判斷錯誤，記下來避免重演：**加回快取這個
commit 後，只用一次「5 秒進入、看起來正常」的測試就判斷「不是
它」，排除得太早。**「用久才出問題」的症狀（例如跟快取 TTL、
連線數累積、配額分鐘窗口有關的問題）本來就不會在單次測試中出現，
排查這類症狀時，至少要測到能觀察出時間模式（例如連續測 5～10 次、
刻意跨過任何已知的 TTL／窗口），才能下「排除」的結論；只測一次
「這次正常」不能當作排除的證據。

排查方法本身：改動一律用 `git revert`／`git revert` 還原 commit 的
方式一個一個加回去或移除（不要用 `git reset --hard`，保留完整歷史，
之後都還能回頭確認），每次改動後都要重新 `clasp push` + `clasp
redeploy`（後端）或等 GitHub Pages 生效（前端）才算真的套用到正式
環境。

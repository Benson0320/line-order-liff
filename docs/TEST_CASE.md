...
一般設計師點刪除，即使前端傳入他人的 designerName，後端仍只能刪除該使用者自己（以 accessToken 驗證出的 userId 為準）的叫貨；非本人的叫貨回傳 `NOT_FOUND`，不得刪除。

## TC-036 叫貨狀況單筆刪除（管理員）

管理員可刪除任意設計師的叫貨；刪除成功後 ITEM_TOTAL 與 LATEST_ORDER_LIST 同步刷新，前端重新整理列表。
